...
def process_batch(batch_df, epoch_id):
    ...
    if not pd_df.empty:
        # Aquí solo imprimimos datos (opcional)
        print(f"\n=== Lote {epoch_id} ===")
        print(pd_df[['city', 'country', 'ts', 'updated_aqi']].head(10))

        # Guardar todos los datos que vienen en el DataFrame (sin alert)
        pd_df.to_sql('air_quality_events', con=mysql_engine, if_exists='append', index=False)

    # Guardar batch en parquet en HDFS (como antes)
    batch_df.write \
        .format("parquet") \
        .mode("append") \
